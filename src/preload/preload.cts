...
contextBridge.exposeInMainWorld("receiptStudio", {
  ...
  save: (collection: string, data: unknown) =>
    invoke("data:save", { collection, data }),
  upsert: (collection: string, entity: unknown) =>
    invoke("data:upsert", { collection, entity }),
  remove: (collection: string, id: string) =>
    invoke("data:remove", { collection, id }),
  completeSale: (sale: unknown, print: boolean) =>
    invoke("sale:complete", { sale, print }),
  ...
  printTemplateTest: (printerId: string, templateId: string) =>
    invoke("print:template-test", { printerId, templateId }),
  printProductLabel: (
    productId: string,
    labelId: string,
    printerId: string,
    copies = 1,
  ) => invoke("print:product-label", { productId, labelId, printerId, copies }),
  printLabelTest: (labelId: string, printerId: string) =>
    invoke("print:label-test", { labelId, printerId }),
  ...
  readShopLogo: (assetId: string) =>
    invoke("asset:read-shop-logo", { assetId }),
