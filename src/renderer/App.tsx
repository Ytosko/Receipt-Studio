...
import { ChevronRight, FileText, Gauge, Moon, Package, Plus, Printer, ReceiptText, Settings as SettingsIcon, ShoppingCart, Store, Sun, Tags, Users } from "lucide-react";
...
function Setup(){
 const {shops,printers,settings,setSettings,upsert}=useStore();const [step,setStep]=useState(0);
 ...
 return <div className="fixed inset-0 z-[60] bg-[#160c2f] text-[#f5efff] flex">
  ...
  <div className="flex-1 flex items-center justify-center p-14"><div className="w-full max-w-xl">{step===0?<><p className="text-[#ff6676] font-bold uppercase tracking-widest text-sm">Welcome</p><h2 className="text-4xl font-bold mt-3">Let’s set up your counter.</h2><p className="text-[#c9bbdf] mt-4 mb-8">A default Rongta RP336UV network profile is ready at 192.168.68.68:9100. First, add your shop.</p><button className="btn btn-primary !bg-[#ff6676] !text-white !border-0 px-6" onClick={()=>setStep(1)}>Create my shop <ChevronRight size={18}/></button></>:<><p className="text-[#ff6676] font-bold uppercase tracking-widest text-sm">Your shop</p><h2 className="text-3xl font-bold mt-2 mb-6">Tell us what goes on the receipt.</h2><ShopForm dark onSave={async shop=>{const template=starterTemplate(shop.id),receiptPrinter=printers.find(printer=>printer.printerType==="receipt");const configured={...shop,defaultTemplateId:template.id,defaultPrinterId:receiptPrinter?.id};await upsert("shops",configured);await upsert("templates",template);await setSettings({...settings,setupComplete:true,activeShopId:shop.id})}}/></>}</div></div>
  <button onClick={finish} className="absolute right-8 top-8 text-sm text-[#9ab0aa] hover:text-white">Skip setup</button>
 </div>
}
export default function App(){
 const store=useStore();const navigate=useNavigate();const load=store.load;useEffect(()=>{void load()},[load]);useEffect(()=>{document.documentElement.classList.toggle("dark",store.settings.theme==="dark")},[store.settings.theme]);
 ...
 return <div className="h-screen flex overflow-hidden">
  ...
  <main className="flex-1 min-w-0 flex flex-col"><header className="h-[68px] bg-[#faf9f5] dark:bg-[#17201f] border-b border-[#deded6] flex items-center justify-between px-7 no-print"><div><p className="text-xs text-[#7c8581]">WORKSPACE</p><p className="font-bold">{active?.name||"No shop configured"}</p></div><div className="flex gap-2"><button className="btn !p-2.5" onClick={()=>void store.setSettings({...store.settings,theme:store.settings.theme==="dark"?"light":"dark"})}>{store.settings.theme==="dark"?<Sun size={17}/>:<Moon size={17}/>}</button><button className="btn btn-primary" onClick={()=>navigate("/sale")}><Plus size={17}/>New sale</button></div></header>
