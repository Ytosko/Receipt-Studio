...
function Setup(){
 ...
 return <div className="fixed inset-0 z-[60] bg-[#160c2f] text-[#f5efff] flex">
  ...
  <div className="flex-1 flex items-center justify-center p-14"><div className="w-full max-w-xl">{step===0?<><p className="text-[#ff6676] font-bold uppercase tracking-widest text-sm">Welcome</p><h2 className="text-4xl font-bold mt-3">Let’s set up your counter.</h2><p className="text-[#c9bbdf] mt-4 mb-8">A default Rongta RP336UV network profile is ready at 192.168.68.68:9100. First, add your shop.</p><button className="btn btn-primary !bg-[#ff6676] !text-white !border-0 px-6" onClick={()=>setStep(1)}>Create my shop <ChevronRight size={18}/></button></>:<><p className="text-[#ff6676] font-bold uppercase tracking-widest text-sm">Your shop</p><h2 className="text-3xl font-bold mt-2 mb-6">Tell us what goes on the receipt.</h2><ShopForm dark onSave={async shop=>{const template=starterTemplate(shop.id);const configured={...shop,defaultTemplateId:template.id,defaultPrinterId:printers[0]?.id};await upsert("shops",configured);await upsert("templates",template);await setSettings({...settings,setupComplete:true,activeShopId:shop.id})}}/></>}</div></div>
