...
import { z } from "zod";
import { collections, labelTemplateSchema, printerSchema, productSchema, saleSchema, type LabelElement, type LabelTemplate, type PrinterProfile, type SaleItem } from "../../shared/schemas.js";
import { maxRedeemablePoints, pointsForSpend, redemptionValue } from "../../shared/loyalty.js";
...
if(process.env.RECEIPT_STUDIO_E2E==="1") app.setPath("userData",path.join(app.getPath("temp"),`receipt-studio-e2e-${process.env.RECEIPT_STUDIO_E2E_RUN||process.pid}`));
...
const returnRequestSchema=z.object({originalSaleId:z.string(),items:z.array(z.object({saleItemId:z.string(),quantity:z.number().int().positive()})).min(1),reason:z.string().trim().min(1),restock:z.boolean().default(true),paymentMethod:z.string().min(1)});
const replacementRequestSchema=returnRequestSchema.extend({replacementItems:z.array(z.object({productId:z.string(),quantity:z.number().int().positive()})).min(1)});
...
async function renderWithAssets(template:any,sale:any,shop:any){
  ...
  if(shop.logoAssetId){
    ...
    if(!source.isEmpty()){
      ...
      const original=source.getSize(), width=Math.min(Math.round(requestedWidth*8),original.width), image=source.resize({width,quality:"best"});
      ...
      lines=lines.map(line=>line.type==="logo"?{type:"image" as const,data:raster,widthBytes,height:size.height,dataUrl,align:line.align,widthMm:line.widthMm}:line);
    }
  }
  return lines;
}
...
async function labelHtml(label:LabelTemplate){
  const elements=await Promise.all(label.elements.map(async(e:LabelElement)=>{
    ...
    if(e.type==="text")return `<div style="${style}font-size:${e.fontSize}px;font-weight:${e.bold?700:400};text-align:${e.align}">${htmlEscape(e.text)}</div>`;
...
async function getLabelPrintSelection(labelId:string,printerId:string){
  ...
  if(!label.savedAt)throw new Error("Save the label template explicitly before printing");
...
function returnedSoFar(sales:any[],originalSaleId:string){
  ...
  for(const transaction of sales.filter(value=>value.originalSaleId===originalSaleId))for(const item of transaction.returnedItems||[])quantities.set(item.saleItemId,(quantities.get(item.saleItemId)||0)+item.quantity);
  return quantities;
}
async function completeSaleTransaction(input:unknown){
  const sale=saleSchema.parse(input),shops=await repo.load("shops") as any[],shop=shops.find(value=>value.id===sale.shopId);
  if(!shop)throw new Error("Shop not found");
  if(!sale.customerId)throw new Error("A customer phone number is required");
  const customers=await repo.load("customers") as any[],customer=customers.find(value=>value.id===sale.customerId);
  if(!customer)throw new Error("Customer not found");
  const products=await repo.load("products") as any[];
  for(const item of sale.items){
    if(!item.productId)continue;
    if(!Number.isInteger(item.quantity))throw new Error(`${item.name} must use a whole-number quantity`);
    const product=products.find(value=>value.id===item.productId);
    if(!product)throw new Error(`${item.name} is no longer available`);
    if(product.stock<item.quantity)throw new Error(`Only ${product.stock} ${item.name} in stock`);
  }
  const loyalty=shop.loyalty,requested=Math.floor((sale.pointsRedeemed||0)/loyalty.redemptionPoints)*loyalty.redemptionPoints;
  const redeemed=Math.min(requested,maxRedeemablePoints(customer.pointsBalance||0,sale.total,loyalty));
  const pointDiscount=Math.min(sale.total,redemptionValue(redeemed,loyalty));
  sale.pointsRedeemed=redeemed;sale.pointDiscount=pointDiscount;sale.total-=pointDiscount;
  sale.pointsEarned=pointsForSpend(sale.total,loyalty);
  customer.pointsBalance=Math.max(0,(customer.pointsBalance||0)-redeemed+sale.pointsEarned);
  customer.updatedAt=new Date().toISOString();sale.pointsBalanceAfter=customer.pointsBalance;
  for(const item of sale.items)if(item.productId){const product=products.find(value=>value.id===item.productId);product.stock-=item.quantity;product.updatedAt=new Date().toISOString();}
  sale.receiptNumber=await repo.reserveReceipt(sale.shopId);
  await repo.save("products",products);await repo.save("customers",customers);await repo.upsert("sales",sale);
  return sale;
}
async function createReturnTransaction(input:unknown,replacement=false){
  const request=replacement?replacementRequestSchema.parse(input):returnRequestSchema.parse(input);
  const sales=await repo.load("sales") as any[],original=sales.find(value=>value.id===request.originalSaleId&&value.transactionType!=="refund");
  if(!original)throw new Error("Original sale not found");
  const used=returnedSoFar(sales,original.id),returnedItems:any[]=[];
  for(const requested of request.items){
    const originalItem=original.items.find((value:any)=>value.id===requested.saleItemId);
    if(!originalItem)throw new Error("Returned item is not part of this sale");
    const available=originalItem.quantity-(used.get(originalItem.id)||0);
    if(requested.quantity>available)throw new Error(`Only ${available} × ${originalItem.name} can be returned`);
    returnedItems.push({saleItemId:originalItem.id,productId:originalItem.productId,name:originalItem.name,quantity:requested.quantity,amount:Math.round(originalItem.lineTotal/originalItem.quantity*requested.quantity)});
  }
  const products=await repo.load("products") as any[],now=new Date().toISOString();
  if(request.restock)for(const item of returnedItems)if(item.productId){const product=products.find(value=>value.id===item.productId);if(product){product.stock+=item.quantity;product.updatedAt=now;}}
  const replacementItems:SaleItem[]=[];
  if(replacement){
    for(const requested of (request as z.infer<typeof replacementRequestSchema>).replacementItems){
      const product=products.find(value=>value.id===requested.productId);if(!product)throw new Error("Replacement product not found");
      if(product.stock<requested.quantity)throw new Error(`Only ${product.stock} ${product.name} in stock`);
      const subtotal=Math.round(product.price*requested.quantity),tax=Math.round(subtotal*(product.taxRate||0)/100);
      replacementItems.push({id:crypto.randomUUID(),productId:product.id,name:product.name,sku:product.sku,quantity:requested.quantity,unitPrice:product.price,discount:0,taxRate:product.taxRate||0,lineSubtotal:subtotal,lineTax:tax,lineTotal:subtotal+tax});
      product.stock-=requested.quantity;product.updatedAt=now;
    }
  }
  const credit=returnedItems.reduce((sum,item)=>sum+item.amount,0),charge=replacementItems.reduce((sum,item)=>sum+item.lineTotal,0),difference=charge-credit;
  const shops=await repo.load("shops") as any[],shop=shops.find(value=>value.id===original.shopId);if(!shop)throw new Error("Shop not found");
  const customers=await repo.load("customers") as any[],customer=customers.find(value=>value.id===original.customerId);
  const priorReversed=sales.filter(value=>value.originalSaleId===original.id).reduce((sum,value)=>sum+(value.pointsReversed||0),0);
  const availableToReverse=Math.max(0,(original.pointsEarned||0)-priorReversed);
  const fullyReturned=original.items.every((item:any)=>(used.get(item.id)||0)+(returnedItems.find(value=>value.saleItemId===item.id)?.quantity||0)>=item.quantity);
  const calculatedReverse=replacement
    ? difference<0?pointsForSpend(-difference,shop.loyalty):0
    : fullyReturned?availableToReverse:pointsForSpend(credit,shop.loyalty);
  const pointsReversed=Math.min(customer?.pointsBalance||0,availableToReverse,calculatedReverse);
  const pointsEarned=difference>0?pointsForSpend(difference,shop.loyalty):0;
  if(customer){customer.pointsBalance=Math.max(0,(customer.pointsBalance||0)-pointsReversed+pointsEarned);customer.updatedAt=now;}
  const transaction=saleSchema.parse({
    id:crypto.randomUUID(),shopId:original.shopId,templateId:original.templateId,printerId:original.printerId,receiptNumber:await repo.reserveReceipt(original.shopId),
    customerId:original.customerId,customerSnapshot:original.customerSnapshot,items:replacementItems.length?replacementItems:returnedItems.map(item=>({id:crypto.randomUUID(),productId:item.productId,name:item.name,quantity:item.quantity,unitPrice:Math.round(item.amount/item.quantity),discount:0,taxRate:0,lineSubtotal:-item.amount,lineTax:0,lineTotal:-item.amount})),
    subtotal:difference,discount:0,tax:0,total:difference,paymentMethod:request.paymentMethod,amountPaid:difference>0?difference:undefined,changeDue:0,note:request.reason,status:"completed",
    transactionType:replacement?"replacement":"refund",originalSaleId:original.id,returnedItems,returnReason:request.reason,restocked:request.restock,
    pointsEarned,pointsReversed,pointsBalanceAfter:customer?.pointsBalance,printStatus:"not_printed",createdAt:now
  });
  await repo.save("products",products);if(customer)await repo.save("customers",customers);await repo.upsert("sales",transaction);
  return transaction;
}
function registerIpc(){
  ...
  ipcMain.handle("data:upsert",async(_,p)=>{
    ...
    if(name==="products"){
      const product=productSchema.parse(p.entity),products=await repo.load("products") as any[];
      const duplicate=product.barcode&&products.find(value=>value.id!==product.id&&value.barcode?.toLowerCase()===product.barcode?.toLowerCase());
      if(duplicate)throw new Error(`Barcode already belongs to ${duplicate.name}`);
  ...
  ipcMain.handle("sale:complete",async(_,p)=>{const parsed=saleSchema.parse(p.sale);if(parsed.printerId){const printers=await repo.load("printers") as any[],printer=printers.find(x=>x.id===parsed.printerId);if(!printer||printer.printerType!=="receipt")throw new Error("POS sales can only use a receipt printer");}const sale=await completeSaleTransaction(parsed);if(p.print){try{return{sale,print:await printSavedSale(sale.id)}}catch(e:any){return{sale,print:{ok:false,message:e.message}}}}return{sale};});
  ipcMain.handle("sale:refund",async(_,p)=>({sale:await createReturnTransaction(p,false)}));
  ipcMain.handle("sale:replace",async(_,p)=>({sale:await createReturnTransaction(p,true)}));
