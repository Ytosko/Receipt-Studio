...
export const shopSchema = z.object({
  id, name: z.string().min(1), legalName: z.string().optional(), addressLines: z.array(z.string()),
...
export const saleItemSchema = z.object({
  ...
  unitPrice: z.number().int().nonnegative(), discount: z.number().int().nonnegative(), taxRate: z.number().min(0),
  lineSubtotal: z.number().int(), lineTax: z.number().int(), lineTotal: z.number().int()
});
export const saleSchema = z.object({
  id, shopId: id, templateId: id, printerId: z.string().optional(), receiptNumber: z.string(), customerId: z.string().optional(),
  customerSnapshot: z.object({ name: z.string(), phone: z.string().optional(), email: z.string().optional(), address: z.string().optional() }).optional(),
...
export const printerSchema = z.object({
  id, name: z.string().min(1), connectionType: z.enum(["network", "system"]),
  printerType: z.enum(["receipt", "label"]).default("receipt"),
  network: z.object({ host: z.string().min(1), port: z.number().int().min(1).max(65535), timeoutMs: z.number().int().min(100) }).optional(),
  system: z.object({ deviceName: z.string() }).optional(), paperWidthMm: z.number().positive(),
  paperHeightMm: z.number().positive().optional(), printableWidthMm: z.number().positive(), characterWidth: z.number().int().min(16).max(100),
  encoding: z.string(), cutAfterPrint: z.boolean(), openDrawerAfterPrint: z.boolean(),
  feedLinesAfterPrint: z.number().int().min(0).max(20),
  dpi: z.union([z.literal(203),z.literal(300),z.literal(600)]).default(203),
  orientation: z.enum(["portrait","landscape"]).default("portrait"),
  commandLanguage: z.enum(["escpos","system","zpl","tspl"]).default("escpos"),
...
export const blockSchema = z.object({
  ...
  size: z.enum(["small","normal","large","xlarge"]).default("normal"), spacingTop: z.number().int().min(0).default(0),
  spacingBottom: z.number().int().min(0).default(0), visibleWhen: z.enum(["always","customer","tax","discount","note","payment"]).default("always"),
  settings: z.record(z.string(), z.unknown()).default({})
});
export const templateSchema = z.object({
  id, shopId: id, name: z.string().min(1), printableWidthMm: z.number().positive(), blocks: z.array(blockSchema),
  createdAt: date, updatedAt: date
});
export const settingsSchema = z.object({ theme: z.enum(["light","dark","system"]).default("light"), setupComplete: z.boolean().default(false), activeShopId: z.string().optional(), paymentMethods: z.array(z.string().min(1)).min(1).default(["Cash","Card","Mobile","Other"]) });
